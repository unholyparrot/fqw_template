...
\begin{document}
    ...
    \input{inc/0-glossary} % Основные понятия
    \input{inc/1-area_review} % Анализ предметной области
    \input{inc/2-implementation} % Реальные проекты
    \input{inc/3-theory} % Теоретическая часть
    \input{inc/5-analysis} % Анализ
    \input{inc/99-conclusion} % Заключение
    
    \input{inc/99-bibliography} % Подключение библиографии
